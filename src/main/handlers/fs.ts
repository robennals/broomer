...
export function register(ipcMain: IpcMain, ctx: HandlerContext): void {
  ...
  ipcMain.handle('fs:readFile', async (_event, filePath: string) => {
    if (ctx.isE2ETest) {
      ...
      if (ctx.isScreenshotMode && filePath.match(/\/tmp\/broomy-review-[^/]+\/review\.json$/)) {
      ...
      if (ctx.isScreenshotMode && filePath.match(/\/tmp\/broomy-review-[^/]+\/comments\.json$/)) {
        return '[]'
      }
  ...
  ipcMain.handle('fs:exists', async (_event, filePath: string) => {
    if (ctx.isScreenshotMode && filePath.match(/\/tmp\/broomy-review-[^/]+\/(review|comments)\.json$/)) {
      return true
    }
