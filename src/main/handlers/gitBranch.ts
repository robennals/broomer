...
async function handleFetchPrHead(ctx: HandlerContext, repoPath: string, prNumber: number) {
  if (ctx.isE2ETest) {
    return { success: true }
  }
...
export function register(ipcMain: IpcMain, ctx: HandlerContext): void {
  ...
  ipcMain.handle('git:fetchPrHead', (_event, repoPath: string, prNumber: number) => handleFetchPrHead(ctx, repoPath, prNumber))
