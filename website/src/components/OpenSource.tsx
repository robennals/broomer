export default function OpenSource() {
  return (
    <section className="px-6 py-20 md:px-12 lg:py-28">
      <div className="mx-auto max-w-[1200px] text-center">
        <h2 className="text-3xl font-semibold tracking-tight text-text-primary md:text-4xl">
          Actually open source
        </h2>
        <p className="mx-auto mt-4 max-w-xl text-text-body leading-relaxed">
          MIT licensed. No telemetry. No paid tier. No &ldquo;open core&rdquo;
          bait-and-switch. Every line of code is on GitHub &mdash; read it, fork
          it, change it, ship your own version.
        </p>

        <div className="mx-auto mt-10 max-w-lg grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
          <div className="rounded-lg border border-border-subtle bg-bg-card p-5">
            <p className="text-sm font-semibold text-text-primary">Fork &amp; modify</p>
            <p className="mt-1 text-sm text-text-muted">
              Don&apos;t like something? Change it. Add your own panels, agents, or workflows.
            </p>
          </div>
          <div className="rounded-lg border border-border-subtle bg-bg-card p-5">
            <p className="text-sm font-semibold text-text-primary">Contribute back</p>
            <p className="mt-1 text-sm text-text-muted">
              Open an issue, send a PR, or start a discussion. The project grows from community input.
            </p>
          </div>
          <div className="rounded-lg border border-border-subtle bg-bg-card p-5">
            <p className="text-sm font-semibold text-text-primary">Runs locally</p>
            <p className="mt-1 text-sm text-text-muted">
              No cloud. No accounts. Your code and sessions stay on your machine.
            </p>
          </div>
        </div>

        <div className="mt-10">
          <a
            href="https://github.com/broomy-ai/broomy"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-lg border border-border-hover px-8 py-4 text-lg font-semibold text-text-primary transition-transform hover:scale-[1.02] hover:border-text-muted"
          >
            <svg
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="currentColor"
              aria-hidden="true"
            >
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
            Browse the source &rarr;
          </a>
          <p className="mt-4 font-mono text-sm text-text-muted">
            github.com/broomy-ai/broomy
          </p>
        </div>
      </div>
    </section>
  )
}
